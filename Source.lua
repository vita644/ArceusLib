...
local Colors = {
    ...
    StrokeColor = Color3.fromRGB(0, 255, 255)  -- Cyan outline
}
...
function Library:CreateWindow(Config)
    ...
    LoadStroke.Color = Colors.StrokeColor
    LoadStroke.Thickness = 2
    
    -- Left side panel
    local LeftPanel = Instance.new("Frame")
    LeftPanel.Parent = LoadingFrame
    LeftPanel.BackgroundColor3 = Colors.SecondBlack
    ...
    LogoLoad.Parent = LeftPanel
    LogoLoad.BackgroundTransparency = 1
    LogoLoad.Position = UDim2.new(0.1, 0, 0.2, 0)
    LogoLoad.Size = UDim2.new(0, 60, 0, 60)
    LogoLoad.Image = Assets.Logo
    
    local TitleLoad = Instance.new("TextLabel")
    TitleLoad.Parent = LeftPanel
    TitleLoad.BackgroundTransparency = 1
    TitleLoad.Position = UDim2.new(0.1, 70, 0.25, 0)
    TitleLoad.Size = UDim2.new(0.8, 0, 0, 30)
    TitleLoad.Font = Enum.Font.GothamBold
    TitleLoad.Text = HubName
    TitleLoad.TextColor3 = Colors.Cyan
    TitleLoad.TextSize = 22
    TitleLoad.TextXAlignment = Enum.TextXAlignment.Left
    
    local DescLoad = Instance.new("TextLabel")
    DescLoad.Parent = LeftPanel
    DescLoad.BackgroundTransparency = 1
    DescLoad.Position = UDim2.new(0.1, 70, 0.45, 0)
    DescLoad.Size = UDim2.new(0.8, 0, 0, 20)
    DescLoad.Font = Enum.Font.Gotham
    DescLoad.Text = Description
    DescLoad.TextColor3 = Colors.Gray
    DescLoad.TextSize = 14
    DescLoad.TextXAlignment = Enum.TextXAlignment.Left
    
    -- Right side loading
    local RightPanel = Instance.new("Frame")
    RightPanel.Parent = LoadingFrame
    RightPanel.BackgroundTransparency = 1
    RightPanel.Position = UDim2.new(0.5, 0, 0, 0)
    RightPanel.Size = UDim2.new(0.5, 0, 1, 0)
    
    local LoadingBar = Instance.new("Frame")
    LoadingBar.Parent = RightPanel
    LoadingBar.BackgroundColor3 = Colors.SecondBlack
    ...
    LoadingFill.BackgroundColor3 = Colors.Cyan
    ...
    LoadingText.TextColor3 = Colors.White
    ...
    local function StartLoading()
        ...
        connection = RunService.Heartbeat:Connect(function()
            percent = math.min(percent + math.random(3, 8), 100)
            LoadingFill.Size = UDim2.new(percent / 100, 0, 1, 0)
            LoadingText.Text = "Loading... " .. percent .. "%"
            
            if percent >= 100 then
                connection:Disconnect()
                TweenService:Create(LoadingFrame, TweenInfo.new(0.6, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Position = UDim2.new(0.5, -300, 0.5, -300)}):Play()
                TweenService:Create(LoadingFrame, TweenInfo.new(0.6), {BackgroundTransparency = 1}):Play()
                task.wait(0.7)
                LoadingFrame:Destroy()
                MainFrame.Visible = true
                -- Floating starts hidden, user can minimize to show it
            end
        end)
    end
    ...
    MainStroke.Color = Colors.StrokeColor
    ...
    AvatarFrame.Position = UDim2.new(0, 15, 1, -70)
    ...
    AvatarStroke.Color = Colors.Cyan
    AvatarStroke.Thickness = 2
    
    local StatusLabel = Instance.new("TextLabel")
    StatusLabel.Name = "StatusLabel"
    StatusLabel.Parent = AvatarFrame
    StatusLabel.BackgroundTransparency = 1
    StatusLabel.Position = UDim2.new(1, 10, 0.5, -8)
    ...
    -- // Floating Icon (Bigger, rounded, with outline)
    local FloatingIcon = Instance.new("ImageButton")
    FloatingIcon.Name = "FloatingIcon"
    FloatingIcon.Parent = ScreenGui
    FloatingIcon.BackgroundColor3 = Colors.MainBlack
    FloatingIcon.BorderSizePixel = 0
    FloatingIcon.Position = UDim2.new(0, 40, 0, 40)
    ...
    FloatStroke.Color = Colors.StrokeColor
    ...
    -- Add stroke to sidebar
    local SidebarStroke = Instance.new("UIStroke", Sidebar)
    SidebarStroke.Color = Colors.StrokeColor
    ...
    Container.Position = UDim2.new(0, 125, 0, 55)
    Container.Size = UDim2.new(1, -130, 1, -60)

    -- // Notification System (Upper right)
    local NotifFrame = Instance.new("Frame")
    NotifFrame.Name = "NotifFrame"
    NotifFrame.Parent = ScreenGui
    NotifFrame.BackgroundTransparency = 1
    NotifFrame.Position = UDim2.new(1, -230, 0, 80)
    NotifFrame.Size = UDim2.new(0, 210, 0, 400)
    ...
    function Library:Notify(title, text, duration)
        ...
        task.delay(duration, function()
            TweenService:Create(Note, TweenInfo.new(0.5), {Position = UDim2.new(1, 30, 0, 0)}):Play()
            task.wait(0.5)
            Note:Destroy()
        end)
    end
